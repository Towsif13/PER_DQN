...
class ReplayBuffer:
    ...
    def update_priorities(self, tds, indices):
        for td, index in zip(tds, indices):
            ...
            data = self.data(updated_priority,
                             updated_probability, updated_weight, index)
        ...
        if self.experience_count > self.buffer_size:
            ...
            if temp.priority == self.priorities_max:
                ...
                self.priorities_max = max(
                    self.memory_data.items(), key=operator.itemgetter(1)).priority
            if self.compute_weights:
                if temp.weight == self.weights_max:
                    self.memory_data[index].weight = 0
                    self.weights_max = max(
                        self.memory_data.items(), key=operator.itemgetter(2)).weight
